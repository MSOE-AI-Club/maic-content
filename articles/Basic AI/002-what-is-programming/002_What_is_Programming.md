...
> MATLAB friendly note: If you mainly program in MATLAB, examples below include short MATLAB equivalents when practical. MATLAB often simplifies numerical and matrix operations and provides built-in toolboxes for signal processing and engineering workflows.
...
```matlab
% MATLAB equivalent - use semicolons to suppress output
x = 5;
```
...
```matlab
% MATLAB function: can be defined in a script or a separate file, e.g., add_numbers.m
function s = add_numbers(a, b)
    s = a + b;
end
```
...
```matlab
% MATLAB class example (in Person.m or inside Live Script)
classdef Person
    properties
        name
    end
...
```matlab
% MATLAB Deep Learning Toolbox equivalent: define layers and a training options
layers = [
    featureInputLayer(4)
...
% Example usage: net = trainNetwork(XTrain, YTrain, layers, options); where XTrain is sized appropriately and YTrain is categorical.
```
